...
def test_get_address_returns_200(client, create_customer, create_address):
    customer = create_customer().get_json()
    created = create_address(customer["id"]).get_json()

    response = client.get(f"/customers/{customer['id']}/addresses/{created['id']}")

    assert response.status_code == 200
    assert response.get_json() == created


def test_get_address_returns_404_when_customer_does_not_exist(client):
    response = client.get("/customers/999999/addresses/1")

    assert response.status_code == 404
    assert response.get_json() == {"error": "customer not found"}


def test_get_address_returns_404_when_address_does_not_exist(client, create_customer):
    customer = create_customer().get_json()

    response = client.get(f"/customers/{customer['id']}/addresses/999999")

    assert response.status_code == 404
    assert response.get_json() == {"error": "address not found"}


def test_get_address_returns_404_when_soft_deleted(
...
def test_get_address_returns_404_when_belongs_to_another_customer(
    client, create_customer, create_address
):
...
def test_get_address_unexpected_error_returns_500_without_leaking_details(
    client, create_customer, create_address, monkeypatch
):
    customer = create_customer().get_json()
    created = create_address(customer["id"]).get_json()

    def _raise(self, customer_id, address_id, *, include_deleted=False):
        raise RuntimeError("boom: connection lost to internal db host")

    monkeypatch.setattr(SQLAlchemyAddressRepository, "get_by_id", _raise)
